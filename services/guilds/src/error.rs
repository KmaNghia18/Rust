...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        let (status, code, message) = match &self {
            ...
            AppError::Database(e)    => { tracing::error!("DB: {:?}", e); (StatusCode::INTERNAL_SERVER_ERROR, "DB_ERROR", "Database error".to_string()) }
            AppError::Internal(e)    => { tracing::error!("Internal: {:?}", e); (StatusCode::INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "Internal error".to_string()) }
        };
