...
where crate::handlers::AppState: axum::extract::FromRef<S> {
    type Rejection = AppError;
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        use axum::extract::FromRef;
        let s = crate::handlers::AppState::from_ref(state);
        ...
        let claims = decode::<Claims>(token, &DecodingKey::from_secret(s.config.jwt_secret.as_bytes()), &val)
            .map(|d| d.claims).map_err(|_| AppError::Unauthorized("Invalid token".into()))?;
        if claims.token_type != "access" { return Err(AppError::Unauthorized("Access token required".into())); }
